    ...
    window.Alpine.data('diffChecker', function ({ newValue, oldValue }) {
        return {
            ...
            calculateDiff() {
                ...
                this.inlineDiffHtml = this.getInlineDiff()
            },
            ...
            generateWordDiff(oldWords, newWords) {
                ...
                while (
                    ...
                    if (
                        ...
                        for (
                            let oldSkip = 0;
                            oldSkip <= maxLookAhead &&
                            oldIndex + oldSkip < oldWords.length;
                            oldSkip++
                        ) {
                            for (
                                let newSkip = 0;
                                newSkip <= maxLookAhead &&
                                newIndex + newSkip < newWords.length;
                                newSkip++
                            ) {
                                if (
                                    oldWords[oldIndex + oldSkip] ===
                                        newWords[newIndex + newSkip] &&
            ...
            generateDiff(oldLines, newLines) {
                ...
                while (
                    ...
                    const oldInLCS =
                        lcs.includes(oldLine) && oldIndex < oldLines.length
                    const newInLCS =
                        lcs.includes(newLine) && newIndex < newLines.length
                    ...
                    if (
                        ...
                        oldIndex < oldLines.length &&
                        (!newInLCS || newIndex >= newLines.length || !oldInLCS)
                    ) {
                        // Line was removed
                        diff.push({
                            type: 'removed',
